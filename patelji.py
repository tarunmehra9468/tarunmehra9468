...
TOKEN = '7264894093:AAFsA8AwNL-EPm04B-ArK8sOts4HtLBMJcU'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -1002243681549
CHANNEL_ID = -1002243681549
error_channel_id = -1002243681549
    ...
    if action == '/approve':
        if plan == 1:  # pateljii 😈
            if users_collection.count_documents({"plan": 1}) >= 99:
                bot.send_message(chat_id, "*Approval failed: pateljii 😈 limit reached (99 users).*", parse_mode='Markdown')
                return
        elif plan == 2:  # patelji 😈
            if users_collection.count_documents({"plan": 2}) >= 499:
                bot.send_message(chat_id, "*Approval failed: patelji 😈 limit reached (499 users).*", parse_mode='Markdown')
                return
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*🚀 Attack Sent Successfully! 😈 https://t.me/patel_ji47 \n\nHost: {target_ip}:{target_port}\nTime: {duration} seconds\nName: {username}*", parse_mode='Markdown')
    except Exception as e:
